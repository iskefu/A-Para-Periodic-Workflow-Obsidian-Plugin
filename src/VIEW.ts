...
export class CreateView extends ItemView {
  ...
  buildUI() {
    const paraWorkflowViewui = document.createElement('div');
    this.containerEl.appendChild(paraWorkflowViewui);
  
    // 创建按钮
    createButtons(paraWorkflowViewui);
    // 默认展示 PARA 面板视图
    const paraButton=paraWorkflowViewui.querySelector('#paraButton')as HTMLElement;
    paraButton.click();
...
function createButtons(container: HTMLDivElement) {
  ...
  button1.addEventListener('click', () => {
    ...
    e.preventDefault();
    const project = (document.querySelector(`#project-name`) as HTMLInputElement).value;
    const area = (document.querySelector(`#area-name`) as HTMLInputElement).value;
    const resource = (document.querySelector(`#resource-name`) as HTMLInputElement).value;
  ...
  button3.addEventListener('click', () => {
    ...
    // 创建备份插件按钮
    const backupPluginButton = createButton('备份插件', '点击此按钮可以备份插件');
    ...
    // 创建提取翻译内容按钮
    const extractTranslationButton = createButton('提取翻译内容', '点击此按钮可以提取翻译内容');
    ...
    // 创建翻译插件按钮
    const translatePluginButton = createButton('翻译插件', '点击此按钮可以翻译插件');
    ...
    // 创建还原插件按钮
    const restorePluginButton = createButton('还原插件', '点击此按钮可以还原插件');
  ...
  button4.addEventListener('click', () => {
    ...
    // 创建自动发布按钮的辅助函数
    function createAutoPublishButton(platformName: string, action: string | symbol) {
        const button = document.createElement('button');
        button.style.padding = '5px 5px';
        button.style.margin = '5px';
    ...
    // 创建微信公众号自动发布按钮
    const weChatButton = createAutoPublishButton('微信公众号', 'WeChatAutoPublishClicked');

    // 创建csdn自动发布按钮
    const csdnButton = createAutoPublishButton('csdn', 'CSDNAutoPublishClicked');

    // 创建知乎自动发布按钮
    const zhihuButton = createAutoPublishButton('知乎', 'ZhihuAutoPublishClicked');

    // 创建简书自动发布按钮
    const jianshuButton = createAutoPublishButton('简书', 'JianshuAutoPublishClicked');

    // 创建掘金自动发布按钮
    const juejinButton = createAutoPublishButton('掘金', 'JuejinAutoPublishClicked');

    // 创建今日头条自动发布按钮
    const toutiaoButton = createAutoPublishButton('今日头条', 'ToutiaoAutoPublishClicked');

    // 创建百家号自动发布按钮
    const baijiahaoButton = createAutoPublishButton('百家号', 'BaijiahaoAutoPublishClicked');

    // 创建哔哩哔哩自动发布按钮
    const bilibiliButton = createAutoPublishButton('哔哩哔哩', 'BilibiliAutoPublishClicked');
