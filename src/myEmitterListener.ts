import { createParaArea, createParaProject, createParaResource } from "func/createPARA";
...
import { BackupPlugins, ExtractTranslationJson, RestorePlugins, tranlatePlugins } from "func/translatePlugins";

export async function myEmitterListener(app: App, settings: MyPluginSettings) {
		//初始化 init 面板
        myEmitter.on('formSubmitted', (e: { area: string; project: string; resource: string; }) => {//监听用户在视图 PARA 面板的输入
